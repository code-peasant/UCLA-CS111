  ...
  while (1)
  {
      ...
      case 'c':
	{
	  ...
	  if (profile == 1)
		  ...
		  struct rusage usage;
		  getrusage(RUSAGE_SELF, &usage);
		  process_array[process_index].m_profile = 1;
		  process_array[process_index].m_user_time = usage.ru_utime.tv_sec+usage.ru_utime.tv_usec/1000000.0;
		  process_array[process_index].m_kernel_time = usage.ru_stime.tv_sec+usage.ru_stime.tv_usec/1000000.0;
		}
	      else
		{
		  process_array[process_index].m_profile = 0;
		}
	  int pid = fork();
	  if (pid == 0)
	    {
      ...
      while (loop_times>0)
	{
	  ...
	  while (i < process_index)
	      ...
	      if (returnpid == process_array[i].pid)
		{
		  ...
		  if (process_array[i].m_profile == 1)
		    {
		      struct rusage usage;
		      getrusage(RUSAGE_SELF,&usage);
		      user_time = usage.ru_utime.tv_sec + usage.ru_utime.tv_usec/1000000.0-process_array[i].m_user_time;
		      kernel_time = usage.ru_stime.tv_sec + usage.ru_stime.tv_usec/1000000.0-process_array[i].m_kernel_time;
		      printf("user_time: %f, kernel_time: %f for ", user_time, kernel_time);
		      int j = 3;
		      while (j<len)
			{
			  printf("%s ",str[j]);
